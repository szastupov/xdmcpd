...
/*
 * Stepan Zastupov: The code is mostly original, I have only changed
 * it to use GChecksum.
 */
		     ...
		     char **bcookiep)
{
	...
	g_checksum_update(sum, (guchar*)randnums, sizeof(int)*RANDNUMS);

	/* use the last cookie */
	g_checksum_update(sum, old_cookie, 16);

	/* use some uninitialized stack space */
	g_checksum_update(sum, (guchar*)cookie, sizeof(cookie));

	pid = getppid ();
	g_checksum_update(sum, (guchar*)&pid, sizeof(pid));
	pid = getpid ();
	g_checksum_update(sum, (guchar*)&pid, sizeof(pid));
	...
	guint8 digest[16] = {0};
	gsize dsize = sizeof(dsize);
	g_checksum_get_digest(sum, digest, &dsize);
	g_checksum_free(sum);
